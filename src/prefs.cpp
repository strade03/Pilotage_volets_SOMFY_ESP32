...
void prefs_set_syslog_state(boolean syslog_state) {
  ...
  write_output_ln("PREFS - Storing syslog state: " + syslog_state ? "true" : "false");
...
boolean prefs_get_syslog_state() {
  ...
  write_output_ln("PREFS - Reading syslog state : ");
  preferences.begin("syslog",false);
  ...
  if (syslog_state)
  {
    write_output_ln("enabled");
  }
  else
  {
    write_output_ln("disabled");
  }
  return syslog_state; 
}
...
void prefs_set_syslog_ip(String syslog_ip) {
  ...
  write_output_ln("PREFS - Storing syslog ip");
  preferences.begin("syslog",false);
...
void prefs_set_syslog_port(int syslog_port) {
  ...
  write_output_ln("PREFS - Storing syslog port");
  preferences.begin("syslog",false);
